#!/bin/bash

# Test the material-scraper function directly
echo "Testing material-scraper function..."

curl -X POST \
  "https://bgbavxtjlbvgplozizxu.supabase.co/functions/v1/material-scraper" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsImtpZCI6Ik1EU1ByZ1hSL2lHdnk2SGUiLCJ0eXAiOiJKV1QifQ..czBx3ny77irDSr1aB266_17WOG50vF9nwX4_08OI3Ak" \
  -H "apikey: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJnYmF2eHRqbGJ2Z3Bsb3ppenh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE5MDYwMzEsImV4cCI6MjA2NzQ4MjAzMX0.xswCBesG3eoYjKY5VNkUNhxc0tG6Ju2IzGI0Yd-DWMg" \
  -d '{
    "url": "https://dimora-shop.com/en/sitemap_products.xml",
    "service": "firecrawl",
    "sitemapMode": true,
    "options": {
      "prompt": "Extract material information from this page. Look for product names, descriptions, prices, images, and material properties.",
      "onlyMainContent": true,
      "includeTags": [".product-card", ".product-item"],
      "excludeTags": [".header", ".footer", ".navigation"]
    }
  }' \
  | jq '.'

echo -e "\n\nTesting single page extraction..."

# Test single page extraction
curl -X POST \
  "https://bgbavxtjlbvgplozizxu.supabase.co/functions/v1/material-scraper" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsImtpZCI6Ik1EU1ByZ1hSL2lHdnk2SGUiLCJ0eXAiOiJKV1QifQ..czBx3ny77irDSr1aB266_17WOG50vF9nwX4_08OI3Ak" \
  -H "apikey: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJnYmF2eHRqbGJ2Z3Bsb3ppenh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTE5MDYwMzEsImV4cCI6MjA2NzQ4MjAzMX0.xswCBesG3eoYjKY5VNkUNhxc0tG6Ju2IzGI0Yd-DWMg" \
  -d '{
    "url": "https://dimora-shop.com/en/products/tiles-kitchen-backsplash",
    "service": "firecrawl",
    "sitemapMode": false,
    "options": {
      "prompt": "Extract material information from this page. Look for product names, descriptions, prices, images, and material properties.",
      "onlyMainContent": true
    }
  }' \
  | jq '.'