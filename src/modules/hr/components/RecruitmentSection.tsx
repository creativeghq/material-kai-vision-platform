...
import {
  hrService, type JobPosting, type Application, type Department, type AppStage, type EmploymentType,
...
function ApplicationsPipeline({ workspaceId, posting, departments, canManage, onBack }: { workspaceId: string; posting: JobPosting; departments: Department[]; canManage: boolean; onBack: () => void }) {
  ...
  const move = async (app: Application, stage: AppStage) => {
    setBusy(app.id);
    try { await hrService.updateApplication(workspaceId, app.id, { stage }); load(); }
    catch (e) { toast({ title: 'Update failed', description: (e as Error).message, variant: 'destructive' }); }
    finally { setBusy(null); }
  };
  const hire = async (app: Application) => {
    setBusy(app.id);
    try { const r = await hrService.hireApplication(workspaceId, app.id); toast({ title: 'Hired 🎉', description: `Employee created + ${r.onboarding_seeded} onboarding tasks seeded.` }); load(); }
    catch (e) { toast({ title: 'Hire failed', description: (e as Error).message, variant: 'destructive' }); }
    finally { setBusy(null); }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={onBack}><ChevronLeft className="h-4 w-4 mr-1" />Jobs</Button>
          <div>
      ...
      {loading ? <Skeleton className="h-48 w-full" /> : apps.length === 0 ? (
        <Card><CardContent><EmptyState icon={UserPlus} title="No applicants yet" hint={canManage ? 'Add candidates as they apply.' : undefined} /></CardContent></Card>
      ) : (
        <div className="grid gap-2">
          {apps.map((a) => (
            <div key={a.id} className="rounded-xl border border-border/60 bg-card p-3 flex items-center gap-3 flex-wrap">
              <div className="min-w-0 flex-1">
                <div className="font-medium truncate">{a.candidate?.name || 'Candidate'}</div>
                <div className="text-xs text-muted-foreground truncate">{a.candidate?.headline || a.candidate?.email || '—'}</div>
              </div>
              {canManage ? (
                <Select value={a.stage} onValueChange={(v) => move(a, v as AppStage)} disabled={busy === a.id || a.stage === 'hired'}>
                  <SelectTrigger className="w-36 h-8"><SelectValue /></SelectTrigger>
                  <SelectContent>{APP_STAGES.map((s) => <SelectItem key={s} value={s}>{APP_STAGE_LABELS[s]}</SelectItem>)}</SelectContent>
                </Select>
              ) : <Badge variant="secondary">{APP_STAGE_LABELS[a.stage]}</Badge>}
              {canManage && a.stage !== 'hired' && a.stage !== 'rejected' && (
                <Button size="sm" className="rounded-full h-8" disabled={busy === a.id} onClick={() => hire(a)}>
                  {busy === a.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><GraduationCap className="h-4 w-4 mr-1" />Hire</>}
                </Button>
              )}
              {a.stage === 'hired' && <Badge variant="default">Hired</Badge>}
            </div>
          ))}
        </div>
      )}
