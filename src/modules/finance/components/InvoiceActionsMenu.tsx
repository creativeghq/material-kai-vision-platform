  ...
  return (
    ...
    <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
      <DialogContent className="sm:max-w-lg" onClick={(e) => e.stopPropagation()}>
        <DialogHeader><DialogTitle>myDATA details</DialogTitle></DialogHeader>
        {detailBusy && !detail ? (
...
function buildHistory(inv: InvoiceWithItems): { label: string; when: string | null }[] {
  ...
  for (const p of inv.payments ?? []) {
    out.push({ label: `Payment ${formatMoney(p.amount, p.currency)} (${(p.method ?? 'other').replace('_', ' ')})`, when: p.paid_at });
  }
