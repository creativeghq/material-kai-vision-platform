...
class SimpleLogger implements ILogger {
  ...
  info(message: string, meta?: unknown): void {
    if (
      ['debug', 'info'].includes(this.config.level) &&
  ...
  warn(message: string, meta?: unknown): void {
    if (
      ['debug', 'info', 'warn'].includes(this.config.level) &&
      this.config.enableConsole
    ) {
...
export class ContainerFactory {
  ...
  private registerBusinessServices(container: IServiceContainer): void {
    ...
    container.register({
      ...
      factory: () => {
        ...
        throw new Error(
          'DocumentChunkingService removed - use mivaaApi.uploadDocument() instead',
        );
      },
      ...
      healthCheck: async (service: unknown) => {
        try {
          ...
          if (
            ...
            typeof (service as { healthCheck: unknown }).healthCheck ===
              'function'
          ) {
            const result = await (
              service as { healthCheck: () => Promise<{ status: string }> }
            ).healthCheck();
    ...
    container.register({
      ...
      factory: () => {
        ...
        } = require('../services/embeddingGenerationService');
    ...
    container.register({
      ...
      factory: () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const {
          MivaaSearchIntegration,
        } = require('../services/mivaaSearchIntegration');
    ...
    container.register({
      ...
      factory: () => {
        throw new Error(
          'MivaaToRagTransformerService implementation not yet available',
        );
      },
      lifetime: ServiceLifetime.Singleton,
    ...
    container.register({
      ...
      factory: () => {
        ...
        } = require('../services/batch/batchProcessingService');
    ...
    container.register({
      ...
      factory: () => {
        throw new Error(
          'ValidationIntegrationService implementation not yet available',
        );
      },
      lifetime: ServiceLifetime.Singleton,
      ...
      healthCheck: async (service: unknown) => {
        try {
          ...
          if (
            service &&
            typeof service === 'object' &&
            'getHealthStatus' in service &&
            typeof (service as { getHealthStatus: unknown }).getHealthStatus ===
              'function'
          ) {
            const result = await (
              service as { getHealthStatus: () => Promise<{ status: string }> }
            ).getHealthStatus();
            return result.status === 'healthy';
          }
  ...
  private registerCoreServices(container: IServiceContainer): void {
    ...
    container.register({
      identifier: 'Logger',
      factory: (resolver) => {
        const configFactory = resolver.resolve<ConfigurationFactory>(
   ...
   */
  public configureForEnvironment(
   ...
   */
  public async validateContainer(
    container: IServiceContainer,
    ...
    try {
      ...
      for (const service of coreServices) {
        const serviceHealth = healthStatus.find(
          (h) => h.identifier === service,
        );
        if (!serviceHealth) {
      ...
      return {
        isValid: false,
        errors: [
          `Container validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        ],
      };
    }
  }
