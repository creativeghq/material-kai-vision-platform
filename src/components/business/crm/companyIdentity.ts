...
/**
 * Every spelling of a VAT number a CRM row might already be stored under, for dedupe reads.
 * A row saved as "EL800370260" must still dedupe against a myDATA issuer_vat of "800370260" —
 * an exact-string match let duplicates straight through (see InboundDocActionsMenu).
 */
export const vatDedupeForms = (vat: string | null | undefined): string[] => {
  const raw = (vat ?? '').trim();
  if (!raw) return [];
  const digits = raw.replace(/\D/g, '');
  return Array.from(new Set([raw, digits, digits ? `EL${digits}` : ''].filter(Boolean)));
};
