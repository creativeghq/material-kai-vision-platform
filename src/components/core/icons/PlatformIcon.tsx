...
export interface PlatformBrand {
  id: string;
  label: string;
  /** The brand's own colour, or null for a mark that must follow the theme's foreground. */
  color: string | null;
...
/**
 * The networks offered on the connect list, in the order they are shown. WhatsApp is in the brand
 * registry but not here — it is a channel you buy a number for, not an account you OAuth into.
 */
export const SOCIAL_PLATFORMS: PlatformBrand[] = [
  'instagram', 'facebook', 'linkedin', 'tiktok', 'pinterest', 'youtube', 'twitter', 'threads',
].map(id => PLATFORM_BRANDS[id]);
...
interface PlatformIconProps {
  platform: string;
  className?: string;
  /** Render in the current text colour rather than the brand's — for dense rows and nav rails. */
  monochrome?: boolean;
}

/**
 * A network's mark. An id nobody recognises (Zernio can return a platform we have no brand for)
 * falls back to a generic link glyph rather than rendering nothing.
 */
export const PlatformIcon: React.FC<PlatformIconProps> = ({ platform, className = 'h-5 w-5', monochrome }) => {
  const brand = PLATFORM_BRANDS[platform];
  if (!brand) return <Link2 className={className} aria-hidden="true" />;
  return (
    <svg
      viewBox="0 0 24 24"
      className={className}
      fill={monochrome || !brand.color ? 'currentColor' : brand.color}
      aria-hidden="true"
      focusable="false"
    >
      <path d={brand.path} />
