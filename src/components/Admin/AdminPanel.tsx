...
export const AdminPanel: React.FC = () => {
  ...
  const calculateStats = (data: AnalyticsEvent[]) => {
    ...
    setStats({
      ...
      avgProcessingTime:
        hybridEvents.length > 0 ? totalProcessingTime / hybridEvents.length : 0,
    });
  };
  ...
  const getProviderBadge = (provider: string) => {
    ...
    if (provider === 'claude')
      return (
        <Badge className="px-2 py-1 bg-gray-200 text-gray-800">Claude</Badge>
      );
    return (
      <Badge className="px-2 py-1 bg-blue-100 text-blue-800">{provider}</Badge>
    );
  };
                      ...
                      <TableRow>
                        <TableCell colSpan={6} className="text-center py-8">
                          <div className="text-muted-foreground">
                            ...
                            No analytics data yet. Try uploading some materials
                            for AI analysis
                          </div>
                        </TableCell>
                      </TableRow>
                      ...
                      <div className="text-2xl font-bold text-green-600">
                        {
                          analyticsData.filter((e) => {
                            const eventData = e.event_data as Record<
                              string,
                              unknown
                            >;
                            return (
                              typeof eventData.final_score === 'number' &&
                              eventData.final_score >= 0.9
                            );
                          }).length
                        }
                      </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">
                          {(
                            ((stats.openaiSuccess + stats.claudeSuccess) /
                              Math.max(stats.totalAnalyses, 1)) *
                            100
                          ).toFixed(1)}
                          %
                        </span>
          ...
          <TabsContent value="rag" className="space-y-4">
            ...
            <div className="text-center p-8">
              <p className="text-muted-foreground">
                RAG Management functionality has been consolidated into the
                Integrated RAG Management panel. Please use the main RAG
                interface for all RAG operations.
              </p>
            </div>
          </TabsContent>
