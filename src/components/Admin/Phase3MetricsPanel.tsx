...
const Phase3MetricsPanel: React.FC = () => {
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      ...
      if (relationships) {
        ...
        for (const rel of relationships) {
          byType[rel.relationship_type] =
            (byType[rel.relationship_type] || 0) + 1;
        ...
        setRetrievalMetrics({
          avg_precision:
            retrievalData.reduce(
              (sum: number, m: any) => sum + m.precision,
              0,
            ) / retrievalData.length,
          avg_recall:
            retrievalData.reduce((sum: number, m: any) => sum + m.recall, 0) /
            retrievalData.length,
          avg_mrr:
            retrievalData.reduce((sum: number, m: any) => sum + m.mrr, 0) /
            retrievalData.length,
          avg_latency_ms:
            retrievalData.reduce(
              (sum: number, m: any) => sum + m.latency_ms,
              0,
            ) / retrievalData.length,
      ...
      const { data: responseData } = await supabase
        ...
        .select(
          'coherence_score, hallucination_score, source_attribution_score, factual_consistency_score, overall_quality_score',
        )
        .order('created_at', { ascending: false })
        .limit(100);
      ...
      if (responseData && responseData.length > 0) {
        setResponseMetrics({
          avg_coherence:
            responseData.reduce(
              (sum: number, m: any) => sum + m.coherence_score,
              0,
            ) / responseData.length,
          avg_hallucination:
            responseData.reduce(
              (sum: number, m: any) => sum + m.hallucination_score,
              0,
            ) / responseData.length,
          avg_attribution:
            responseData.reduce(
              (sum: number, m: any) => sum + m.source_attribution_score,
              0,
            ) / responseData.length,
          avg_consistency:
            responseData.reduce(
              (sum: number, m: any) => sum + m.factual_consistency_score,
              0,
            ) / responseData.length,
          avg_overall:
            responseData.reduce(
              (sum: number, m: any) => sum + m.overall_quality_score,
              0,
            ) / responseData.length,
  ...
  const overallHealth =
    ...
    ((retrievalMetrics.avg_precision + retrievalMetrics.avg_recall) / 2) *
      0.35 +
          ...
          <TabsContent value="relationships" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Sequential, semantic, and hierarchical relationships
                </CardDescription>
              </CardHeader>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="text-3xl font-bold">
                      {relationshipStats.total_relationships}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm font-medium mb-2">
                    ...
                    <div className="text-3xl font-bold">
                      {(relationshipStats.avg_confidence * 100).toFixed(1)}%
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {relationshipStats.sequential}
                    </div>
                    <Progress
                      value={
                        (relationshipStats.sequential /
                          Math.max(relationshipStats.total_relationships, 1)) *
                        100
                      }
                      className="mt-2 h-2"
                    />
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {relationshipStats.hierarchical}
                    </div>
                    <Progress
                      value={
                        (relationshipStats.hierarchical /
              ...
              <CardContent className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <div className="text-3xl font-bold">
                      {retrievalMetrics.total_queries}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm font-medium mb-2">Avg Latency</div>
                    <div className="text-3xl font-bold">
                      {retrievalMetrics.avg_latency_ms.toFixed(0)}ms
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(retrievalMetrics.avg_precision * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={retrievalMetrics.avg_precision * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(retrievalMetrics.avg_recall * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={retrievalMetrics.avg_recall * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                </div>
                ...
                <div className="border rounded-lg p-4">
                  ...
                  <div className="text-2xl font-bold mt-2">
                    {retrievalMetrics.avg_mrr.toFixed(3)}
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <TabsContent value="response" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  LLM output quality and validation
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="text-3xl font-bold">
                      {(responseMetrics.avg_overall * 100).toFixed(1)}%
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(responseMetrics.avg_coherence * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={responseMetrics.avg_coherence * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(responseMetrics.avg_hallucination * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={responseMetrics.avg_hallucination * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(responseMetrics.avg_attribution * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={responseMetrics.avg_attribution * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="text-2xl font-bold mt-2">
                      {(responseMetrics.avg_consistency * 100).toFixed(1)}%
                    </div>
                    <Progress
                      value={responseMetrics.avg_consistency * 100}
                      className="mt-2 h-2"
                    />
                  </div>
                </div>
