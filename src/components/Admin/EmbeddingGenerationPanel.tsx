...
const EmbeddingGenerationPanel: React.FC = () => {
  ...
  const [embeddingStats, setEmbeddingStats] = useState<
    Array<{ model_name: string; count: number }>
  >([]);
  ...
  const fetchEmbeddingStats = useCallback(async () => {
    try {
      ...
      setEmbeddingStats(
        Object.entries(modelCounts).map(([model_name, count]) => ({
          model_name,
          count,
        })),
      );
      ...
      const embeddingTypeCounts: Record<
        string,
      ...
      const embeddingTypes = Object.entries(embeddingTypeCounts).map(
        ([type, data]) => ({
          type,
          count: data.count,
      ...
      const entriesWithEmbeddings =
        knowledgeData?.filter((entry: any) => entry.embedding).length || 0;
  ...
  const materialCoverage = stats
    ? (stats.materialsWithEmbeddings / stats.totalMaterials) * 100
    : 0;
  const knowledgeCoverage = stats
    ? (stats.entriesWithEmbeddings / stats.totalKnowledgeEntries) * 100
    : 0;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitor and manage vector embeddings for materials and knowledge
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            onClick={fetchEmbeddingStats}
            onKeyDown={(e) => e.key === 'Enter' && fetchEmbeddingStats()}
          ...
          <Button
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleRegenerateEmbeddings()}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {stats?.materialsWithEmbeddings || 0}
                </p>
                <p className="text-xs text-muted-foreground">
                  of {stats?.totalMaterials || 0} total
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {stats?.entriesWithEmbeddings || 0}
                </p>
                <p className="text-xs text-muted-foreground">
                  of {stats?.totalKnowledgeEntries || 0} total
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Active embedding models and their usage statistics
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Current embedding generation system health
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 p-3 bg-muted rounded-lg">
            <p className="text-sm text-muted-foreground">
              Embedding generation is handled automatically during material
              processing. Custom models will be deployed here after training
              completion.
            </p>
          </div>
        </CardContent>
