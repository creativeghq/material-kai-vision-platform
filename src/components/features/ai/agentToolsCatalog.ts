...
const KAI_TOOLS: AgentToolEntry[] = [
  ...
  // ── Reviews (module-gated; per-user via RLS; public reply confirm-gated) ──
  {
    id: 'manage_reviews', name: 'Reviews', category: 'Reviews',
    moduleSlug: 'reviews',
    desc: 'List reviews written about you and post a public reply (a reply asks for Approve/Decline first).',
    examples: [
      'Show reviews I haven\'t replied to',
    ...
    quick_starts: [
      ...
      },
      {
        ...
        form: [
          ...
          { key: 'platform', label: 'Publish to', kind: 'select', required: true, default: 'instagram', options: [
    ...
    quick_starts: [
      {
        ...
        prompt: 'Show me the cost report for this project.',
        done: 'Here is where that job stands.',
        promptTemplate: 'Show me the cost value reconciliation for the project "{{project_name}}".',
        run: { tool: 'project_cvr' },
        autoFields: true,
      },
      {
        label: 'What are we owed?', description: 'Applications, certifications and retention', icon: 'Receipt',
        prompt: 'What are we owed on this project?',
        done: 'That is what is outstanding.',
        promptTemplate: 'Show the payment applications and retention for the project "{{project_name}}".',
        run: { tool: 'project_applications' },
        autoFields: true,
      },
      {
        label: 'Variations not agreed', description: 'Changes still waiting on a decision', icon: 'GitPullRequestArrow',
        prompt: 'Which variations have not been agreed yet?',
        done: 'Those are the ones still open.',
        promptTemplate: 'List the undecided variations on the project "{{project_name}}".',
        run: { tool: 'list_variations', fixedArgs: { only_undecided: true } },
        autoFields: true,
      },
      {
        label: 'Where are the tenders?', description: 'Who has been asked, who has replied', icon: 'Gavel',
        prompt: 'Where have the tender packages got to?',
      ...
      },
      {
        ...
        form: [
          ...
          { key: 'model', label: 'Model', kind: 'select', default: 'veo-2', options: [
            ...
            { value: 'wan-3.0-480p', label: 'Wan3.0 480p — 30s with audio, 30 credits' },
            { value: 'wan-3.0-720p', label: 'Wan3.0 720p — 30s with audio, 55 credits' },
            { value: 'wan-3.0-1080p', label: 'Wan3.0 1080p — 30s with audio, 110 credits' },
            ...
            { value: 'h3-max-768p', label: 'H3 Max 768p — 15s with audio, 25 credits' },
            { value: 'h3-max-480p', label: 'H3 Max 480p — 15s with audio, 15 credits' },
          ...
          { key: 'duration_seconds', label: 'Duration (seconds)', kind: 'number', default: '8', help: '4–30 seconds; clamped to the model ceiling (Veo 8s, Kling 10s, H3 Max 15s, Wan/Seedance 30s)' },
...
export const TOOLKIT_HUB: Record<string, HubId> = {
  ...
  // Studio
  catalogs: 'studio', 'presentation-sheets': 'studio', projects: 'studio', construction: 'studio', generation: 'studio',
