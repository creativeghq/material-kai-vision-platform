...
import {
  PDFWorkflowViewer,
  WorkflowJob,
} from '@/components/PDF/PDFWorkflowViewer';
...
const PDFProcessing = () => {
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        const cats =
          await dynamicCategoryManagementService.getCategoriesHierarchy();
        setCategories(cats);
        if (cats.length > 0) {
    ...
    // Load existing jobs
    setWorkflowJobs(
      consolidatedPDFWorkflowService.getAllJobs() as WorkflowJob[],
    );
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    for (const file of acceptedFiles) {
      try {
        const jobId = await consolidatedPDFWorkflowService.startPDFProcessing(
          file,
          {
        ...
        const job = consolidatedPDFWorkflowService.getJob(jobId) as
          | WorkflowJob
          | undefined;
        if (job) {
          setCurrentJob(job);
  ...
  const processUrlPdf = async () => {
    ...
    try {
      ...
      const jobId =
        await consolidatedPDFWorkflowService.startPDFProcessing(file);
      ...
      toast({
        title: 'Processing Started',
        description:
          'Started processing PDF from URL. You can monitor the progress in the modal.',
      });
  ...
  const runningJobs = workflowJobs.filter(
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Upload files or enter a URL to start the complete processing
                  workflow with real-time monitoring
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Select the material category for this PDF to help organize
                  ...
                  {isDragActive ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Complete workflow with authentication, upload,
                        validation, processing, and knowledge base storage
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  ...
                  <div className="text-sm text-blue-800 space-y-1">
                    <p>
                      • <strong>Small files (&lt;20MB):</strong> Fast
                      synchronous processing (1-3 minutes)
                    </p>
                    <p>
                      • <strong>Large files (&gt;20MB):</strong> Automatic async
                      processing (no timeouts)
                    </p>
                    <p>
                      • <strong>Complex documents:</strong> Signature books,
                      forms with many images supported
                    </p>
                    <p>
                      • <strong>Real-time updates:</strong> Progress tracking
                      for all processing modes
                    </p>
                    <p>
                      • <strong>No size limits:</strong> Process PDFs of any
                      size with our async system
                    </p>
                  </div>
                </div>
              </CardContent>
