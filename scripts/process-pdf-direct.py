...
Bypasses MIVAA's buggy implementation for large PDFs
"""
...
def process_pdf_with_pymupdf(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print("❌ pymupdf4llm not available, trying alternative method...")
        return process_pdf_with_pypdf(pdf_path)
    except Exception as e:
        print(f"❌ Error processing PDF with pymupdf4llm: {e}")
        return process_pdf_with_pypdf(pdf_path)

def process_pdf_with_pypdf(pdf_path: str) -> Dict[str, Any]:
    """Fallback: Process PDF using PyPDF2"""
    try:
        import PyPDF2
        print("📄 Processing PDF with PyPDF2 (fallback)...")
...
def process_pdf_complete(url: str, filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        page_count = markdown_content.count('--- Page') if '--- Page' in markdown_content else 1
