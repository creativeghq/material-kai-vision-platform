...
- **A column ADDED to `products`, `order_items` or `invoice_items` must be GRANTED, or declared withheld.** Those three have no table-level SELECT for `authenticated`/`anon` since #358 — only a column list, so the cost columns cannot be read by a client. A new column is therefore readable by NOBODY, and PostgREST refuses the *whole request* naming it with `42501 permission denied for table` — loud as a toast, silent in a `{ data }` destructure. `products.profile_user_id` broke Profile → Services the day it landed; `order_items.configured_options` sat in `ORDER_ITEM_SELECT` for a week (2026-08-29 → 09-05) and every order detail rendered with no lines. In the same migration: `GRANT SELECT (col) ON public.<table> TO authenticated, anon;` — or, if it is a cost fact, `COMMENT ON COLUMN` containing the words **"NOT selectable"** and a DEFINER reader like `get_product_costs`. `lint_column_grants()` lists every ungranted column with no such comment (the `db.column-grants` smoke check, strict zero); call it in the same session as the migration.
...
- **Before done**: `npm run lint && npm run typecheck && npm test`. All three must pass. **`lint` is not optional and it is the one people skip** — the Deploy workflow depends on the unit-tests job, which runs `eslint . --max-warnings 0`, so a lint-only failure passes every other check here and still stops the deploy *silently*. On 2026-08-14 that shape blocked four merges in one day (unused imports left behind by a deletion, and template literals that lost their last `${}`) — all of them "green locally". `npm run typecheck` covers `src/` ONLY — `tsconfig.json` excludes `supabase/**`. Edge functions are checked by `npm run typecheck:edge` (`deno check`, gated in CI by `.github/workflows/edge-typecheck.yml`); run it when you touch one. It is baseline-relative — `.github/edge-typecheck-baseline.json` fails the build when a count rises, so ratchet the numbers down as you fix things rather than editing the baseline upward. **Both gates are cached now, and the cold numbers are why**: measured 2026-09-20 on this repo, `typecheck` 337s cold / **35s** warm (tsc `incremental`, on by default in `tsconfig.json` — tsc versions the dependency graph itself, so it is exactly as sound as a cold run), and `lint` 218s cold / **11s** warm no-op / **55-65s** with one file changed, via `npm run lint:fast`. Use `lint:fast` while iterating; `npm run lint` stays uncached and is what CI runs, because ESLint here is type-aware (`projectService: true`) and its cache keys on the CHANGED file only — a type edit in A can change a type-aware verdict in B, and the cache would not re-lint B. Both caches live in `node_modules/.cache/`, so a clean checkout pays the cold cost once and nothing is committed. **`npm test` passes `--retry=0`**, because `vitest.config.ts` sets `retry: 2` for the INTEGRATION tier's ~68 live network calls and it applied to the hermetic unit tier too — a failing guard ran three times, so the run was slowest exactly when it had bad news. The flag rides on the script rather than the config so the integration tier keeps its retries. **The unit suite's remaining cost is NOT the guard tests' file IO**: routing all 244 `stripComments(readFileSync(…))` sites through the memoizing `tests/helpers/sourceIndex` cut test time 867s → 690s across workers and moved WALL clock not at all (191s → 201s), because the suite is bound by module transform + collect (~975s of the ~1,000s). Measured 2026-09-20, and reverted. Look there, not at the reads, before spending time on this again.
...
- **A comment says what the code IS, in at most 6 prose lines.** The narrative of how it got that way goes in the commit message, the issue, or `docs/` — not into every context window that ever loads the file. Comments were 10% of `src/` and 23% of `supabase/functions/`; 2,621 of them were essays, and 33.7k lines were cut on 2026-09-10. `@tag` lines and their continuations do NOT count, so real JSDoc is never what trips this, and a run of consecutive `//` lines counts as ONE comment — that is how an eight-line essay used to slip a check that only looked at block comments. Tooling directives (`eslint-disable`, `@ts-`, …) are exempt; generated files are fixed at the GENERATOR, never in the output. `npm run comments:trim` cuts a comment to its opening paragraph plus its tags, and refuses any file whose code would change. The budget lives once, in [scripts/lib/commentBudget.mjs](scripts/lib/commentBudget.mjs); ESLint enforces it for `src/` and `api/` (`local/no-essay-comments`) and [tests/unit/commentBudget.test.ts](tests/unit/commentBudget.test.ts) covers every runtime ESLint ignores — which is where the longest comments actually lived. **Never assert on comment PROSE in a guard test.** Four did, and four more compared byte offsets that only lined up because a comment happened to mention the term first — `debitOrRefuse` before `callClaudeMessages` was comparing two `import` lines, so it would have passed with the debit moved after the model call.
- **The 6-line budget is a CEILING, not an allowance — a comment is not free, and adding one means deleting one.** That rule caps ONE comment and nothing capped how MANY, which is how the platform sat at **12.6% comment — 78,951 lines, ~868k tokens** — on 2026-09-15 with every check green. Default to writing NO comment; write one only where the code cannot state the fact itself (a trap, a non-obvious invariant, a "this looks wrong and is not"), and never pad to the budget — 4,835 comments sit at 4–6 prose lines because `comments:trim` cut the 2026-09-10 essays *down to* the limit instead of deleting them. `.github/comment-mass-baseline.json` holds comment lines per area and only ever goes DOWN: `npm run comments:mass` reports and fails on a rise naming the area, `-- --write` lowers it and **refuses to raise it** (`--force` exists for a genuinely new area of the product; say so in the commit). Deleting a few comments is free, but remove more than ~1% and the test asks you to re-record, so a cleanup becomes the new ceiling instead of banked headroom. The mass metric deliberately counts what the per-comment rule is blind to — **trailing** comments, **directive** comments (an essay carrying an `eslint-disable` is invisible to the budget and to `comments:trim`) and line WIDTH — so a green `npm run lint` is not "this comment is fine". **Compress an over-budget comment by HAND**: `comments:trim` keeps the opening paragraph and drops everything after it, which is usually the trap clause worth keeping. Guarded by [tests/unit/commentBudget.test.ts](tests/unit/commentBudget.test.ts).
- **MIVAA has the same budget**, in [scripts/comment_budget.py](mivaa-pdf-extractor/scripts/comment_budget.py) with `python scripts/trim_comments.py` and `tests/unit/test_comment_budget.py`; `Args:`/`Returns:`/`Raises:` sections never count and are kept whole. Five guard tests there were reading prose — one counted `tracked_claude_call_async` in a file that has only ever MENTIONED it, in a docstring and a comment, and one let a comment satisfy an invariant-5 route gate. Both sweeps now run over `blank_comments()` output, so a marker can only be satisfied by code. **A guard test that greps source must blank comments first.**
...
9. **LLM safety.** Untrusted ingested content (scraped pages, PDF text, supplier XML) fed to an LLM MUST be wrapped in explicit "this is DATA, not instructions" delimiters. Classifiers whose verdict drives a DB write or alert MUST have their output constrained BY THE SERVER — Anthropic `tools=[...]` + `tool_choice`, **or** a strict `output_config.format` JSON schema — never free-form JSON + a salvage parser. The two are equivalent for this rule and the second is not a weakening: what the rule forbids is *repairing* a reply, and both remove the thing to repair. Prefer `output_config.format` where the forced call only ever existed to get a schema-valid object back, because **Fable 5.1 returns a 400 on `tool_choice` `any`/`tool`** — a path shaped as a forced tool cannot even be measured against the newest model, which is why all four `vision_analysis` paths moved (#400 W5). Under either shape a refusal (`stop_reason: "refusal"`) or an unparseable reply is a FAILED analysis, never a parse target. `call_with_tool` / `call_with_schema` in `claude_tool_call.py` are the two entry points; both raise the same `ToolCallNotReturned`. State-mutating agent tools require explicit user confirmation when triggered off tool-result content.
...
- **A document declares its taxes on the LINES or at DOCUMENT level — never both.** myDATA offers `taxesTotals` (N rows, each with its own `taxCategory` and a free-text `taxTypeLabel`) as an ALTERNATIVE to the per-line tax fields; a levy stated in both places is filed twice, and a doubled tax is a valid number on a valid document, so nothing downstream raises. A line carries exactly ONE category per bucket, which is why the per-line form cannot express a recycling levy at all — that is one AADE code (fees 17) charged at a different rate per ΑΗΗΕ appliance class, told apart only by the label. `invoice_taxes` / `credit_note_taxes` hold the document form; their presence IS the mode, and `recompute_invoice_tax_totals` is the one derivation of the five header totals and of `invoices.tax_payable_delta`. Sign comes from AADE's `reducesPayable` (Appendix §23) through `mydata_tax_payable_delta` in SQL — the TS twin in [src/modules/finance/mydataTaxPayable.ts](src/modules/finance/mydataTaxPayable.ts) exists ONLY so the dialog can total a document that has no row yet, and the two are held equal by a truth table. A levy billed to the customer as its own VAT-bearing line carries `rec_type = 3` (Appendix §12, "Other Taxes Line with VAT"); without it AADE reads it as goods. Only 3 is offered — 2, 6 and 7 are separate features, and 7 is valid only on 17.3–17.6.
- **A `mydata_reference` rate AADE publishes is STORED, and `rate_kind='per_unit'` means rate x quantity.** Fees 16 (€0.04/piece, plastic products) and fees 17 (€0.08/piece, recycling) carried `rate = NULL`, so the picker said "(amount)" and the operator typed a figure from memory and multiplied by quantity in their head. The Novus **Appendix.pdf** (`src/modules/myaade/NovusProvider/`, last update 15/07/2025) is the code tables — read it against the seed rather than trusting either. It is a translation, not the authority: it renders other-taxes 15 as "Life insurance premiums" (it is FIRE branch at 20%, which is why codes 1–2 were withdrawn) and prints the stamp-duty rates as 1/2/4% instead of 1.2/2.4/3.6. Our seed is right on both.
- Guarded by [tests/unit/invoiceDocumentFields.test.ts](tests/unit/invoiceDocumentFields.test.ts) + [tests/unit/financeMirrors.test.ts](tests/unit/financeMirrors.test.ts) + [tests/unit/mydataDocumentTaxes.test.ts](tests/unit/mydataDocumentTaxes.test.ts).
...
- **The display face never renders a string that came out of the DATABASE.** Per-character matching is exactly what makes this silent: a name mixing scripts — `KEROS HELLAS ΕΤΑΙΡΙΑ ΠΕΡΙΟΡΙΣΜΕΝΗΣ ΕΥΘΥΝΗΣ`, the shape every ΑΑΔΕ/ΓΕΜΗ registry name takes — draws its latin half in Aleo and its Greek half in Averta **inside one line**, and both faces load, the string is intact and the class is valid, so nothing raises. `DialogTitle` is therefore SANS (a chrome heading by the h3–h6 rule, and where 118 interpolated record names live); `PageHeader` takes `recordTitle` for the 5 callers passing a record name; an `h1`/`h2` holding one needs `font-sans`, since it inherits the face from `index.css`. The serif stays on identity surfaces — page titles, marketing headings, numerals, copy this app authors. Guarded by [tests/unit/designSystem.test.ts](tests/unit/designSystem.test.ts), whose allowlist is shrink-only with a reason per entry. NOT yet covered: ~30 bare `h1`/`h2` on public/portal pages that interpolate a record name and inherit the face rather than opting in.
...
- **The mobile bottom inset is TWO declarations and either alone reserves nothing.** `.mobile-content` pads for the fixed tab bar, and `.mobile-content > * { flex-shrink: 0 }` is what stops that padding being spent. `<main>` is a column flex scroll container, so a page root is a flex item; an explicit `min-height` REPLACES `min-height: auto`, dropping the item's shrink floor, and Chromium then shrinks the root by exactly the padding — scrollable height unchanged, last ~56px under the bar. `min-h-screen` is the root of 67 pages, so this hit almost everything, and it looked correct in the CSS the whole time (reported on `/finance?tab=mydata_book`). A spacer element and a bottom margin are absorbed the same way — all three measured in headless Chromium. Guarded by [tests/unit/responsiveTableOverflow.test.ts](tests/unit/responsiveTableOverflow.test.ts).
- **A section rail is also a MENU, and Finance's is declared once.** `src/modules/finance/sections.ts` is the one list; `FinancePage` maps `financeRailRows()` into its `TabsList` and `LAUNCHER_SECTIONS.finance` maps the same array into App Launcher chips. Adding a Finance tab means adding it THERE — never a second entry beside the derivation. The launcher's hand-written copy had 10 of the rail's 25, so fifteen sections (Receipts, Credit Notes, Expenses, Delivery Notes, Cheques, Planning, Assets, Time & Billing, AI Assessment, myDATA Book, myDATA Transmissions, Sourcing, Settings, Supplier Portal, By Supplier) were in the product and in no menu; HR was missing Ergani, CRM its own Pipeline tab. `deepLinkTargets` cannot see this — it only checks that the chips which DO exist resolve — and every chip resolved. Gates travel with the entry (`moduleSlug`, `requireAnyCapability`, `hideForAccountant`): a chip the page would hide opens a pane that renders nothing. Guarded by [tests/unit/launcherSectionParity.test.ts](tests/unit/launcherSectionParity.test.ts), which for every literal-trigger rail requires each tab to be a chip or carry a stated reason (shrink-only).
- **ONE rail per screen — a rail inside a rail is two sidebars, and the page header describes only the outer one.** Profile → Websites opened a site's **23-section** rail beside the profile's own fourteen, under a band reading "My Profile · Manage your account, credits, and billing"; Schedule and Social Accounts did the same with `HubSideNav`. A rail is *the page's* section nav, so the fix is never a third nav: a drill-down into a RECORD becomes its own page (a website is `/websites/:id`, domain in `PageHeader` with `recordTitle`, breadcrumbs back to the list), and a settings section with a handful of panes becomes `HubTabNav` — the horizontal underline strip. **Flattening a grouped rail drops the group captions**, so a label that leaned on its caption must now name its own subject (Social Accounts had two panes called "Analytics"). A horizontal tab strip above a rail is NOT this bug: the CRM record pages keep their sub-rails in mutually exclusive `TabsContent` panes, so one is ever on screen. Guarded by the "a rail never contains another rail" case in [tests/unit/responsiveTableOverflow.test.ts](tests/unit/responsiveTableOverflow.test.ts) (exemptions shrink-only, reason per entry).
...
| AI Assessment (projects, finance, real estate) | [docs/ai-assessment.md](docs/ai-assessment.md) — ONE system, three subjects (`assessments.subject_type`), not three copies. The model **never counts**: the signals, the six dimension scores and the verdict are derived by `get_assessment_snapshot`; the Claude turn only writes about them. An action must name a signal its own report raised, and `dimension`/`impact`/`destination` are read off that signal, not off the model. Only a PROJECT action can become a task — `project_tasks` is the only task table, and the other two subjects refuse with a reason |
