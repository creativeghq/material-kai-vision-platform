...
- **Endpoint manager**: [surya_endpoint_manager.py](mivaa-pdf-extractor/app/services/pdf/surya_endpoint_manager.py) mirrors the old Chandra lifecycle (resume/warmup/scale-to-zero, HF billing fast-fail) + `run_structural_pass`/`run_block_ocr`, retry-with-jitter, 2MP downscale. Per-attempt telemetry → `surya_ocr_metrics` table. Pricing: `SURYA_PRICING` in `ai_pricing.py`; admin monitor (`AsyncJobQueueMonitor`, `PlatformOverviewTab`) surfaces Surya, not YOLO/Chandra. Secrets: `SURYA_ENDPOINT_URL`/`SURYA_ENDPOINT_NAME`/`SURYA_NAMESPACE` (defaults baked into `config.py`).
