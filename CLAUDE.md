...
- **Theme**: Default is **LIGHT — "Graphite + Lime"** (2026-06-14). Light page is deep warm graphite off-white (`--background: 82 13% 88%`, near-white cards `80 33% 99%`), ink `--primary` (`220 10% 12%` → black pills), electric-lime accent/focus (`--ring: 80 72% 44%`), muted-sage `--secondary` (`133 8% 54%` = `#809284`). Dark mode (near-black `0 0% 7%`, light text, plum primary) remains fully maintained as a toggle. Light tokens live in the `html.light` block, dark in `:root` — both in `src/index.css`. `.dashboard-card` carries a real resting shadow (`--glass-shadow`) so the system reads elevated, not flat.
- **No hover "float"**: cards/grids/list items must NOT lift, zoom, or gain a shadow on hover (user preference). Enforced globally at the end of `src/index.css` — un-layered rules cancel `hover:shadow-*` / `hover:-translate-y-*` / `hover:scale-*` on everything except `<button>` (intentional) and `<img>` (group-hover image zoom kept). Don't reintroduce hover-elevation on content cards.
- **Primary**: light = ink (`220 10% 12%`); dark = brightened plum (`330 50% 35%`). **Accent**: light = pale-lime tint + lime focus ring; dark = warm (`22 60% 18%`). **Secondary**: light = muted sage `#809284`.
