...
## PDF Pipeline — Surya-2 structural backbone (2026-06-13, structure-first)
The catalog pipeline's layout+OCR front-end was replaced by a single **Surya-2** vision-language model (`datalab-to/surya-ocr-2`, HF Inference endpoint, eu-west-1). **YOLO, Chandra, and `merge_layout` were deleted** — `yolo_endpoint_manager.py`, `yolo_layout_detector.py`, `chandra_endpoint_manager.py`, `layout_merge_service.py` are gone, along with all `yolo_*`/`chandra_*` Settings, `get_yolo_config`/`get_chandra_config`, registry `get_yolo_manager`/`get_chandra_manager`, and the controller/auto-scaler/health entries. The two warmed HF endpoints are now **SLIG + Surya**.
- **One Surya call per page** returns layout regions (label + bbox) + OCR'd text + figure boxes — what YOLO (boxes) + Chandra (text) + merge (bucketing) did in three steps. Contract: full-page `high_accuracy_bbox` mode → top-level `<div data-label= data-bbox=>` blocks, **bbox 0-1000 normalized** (divided to 0..1 at the parser boundary in [surya_blocks.py](mivaa-pdf-extractor/app/services/pdf/surya_blocks.py)). The 19 Surya labels map onto the existing `region_type` vocab; `Image`/`Figure`/`Diagram` are the product-crop sources.
- **Structure-first ordering**: the Surya pass runs as **Stage 1, BEFORE discovery** ([rag_routes.py](mivaa-pdf-extractor/app/api/rag_routes.py) `process_document_with_discovery`). It persists `document_layout_analysis` rows with `processing_version="surya-2"`. Discovery then reads Surya's reading-order text from that cache (via `build_page_text_from_layout_cache`) instead of raw `page.get_text()`; Stage 2 chunking + Stage 3 crops read the same cache (reader gates in `pdf_processor`/`stage_1_focused_extraction` now accept `surya-2`). No per-product YOLO fallbacks remain.
- **OCR is Surya too**: [ocr_service.py](mivaa-pdf-extractor/app/services/pdf/ocr_service.py) (`_call_surya` via `run_structural_pass` on the crop) backs Phase-3 per-image OCR + icon metadata + the admin re-OCR endpoint; `OCRResult.method` is `surya`/`surya_failed`. `ocr_engine` setting = `surya`.
- **Endpoint manager**: [surya_endpoint_manager.py](mivaa-pdf-extractor/app/services/pdf/surya_endpoint_manager.py) holds the `run_structural_pass`/`run_block_ocr` inference + retry-with-jitter + 2MP downscale + `surya_ocr_metrics` telemetry. It is **provider-agnostic** — the endpoint **lifecycle** (warmup/resume/scale-to-zero) is delegated to an `EndpointProvider`. Pricing: `SURYA_PRICING` in `ai_pricing.py`; admin monitor (`AsyncJobQueueMonitor`, `PlatformOverviewTab`) surfaces Surya, not YOLO/Chandra.
- **GPU host is a switch — Modal (default) OR HuggingFace (`SURYA_PROVIDER`)**: [endpoint_providers.py](mivaa-pdf-extractor/app/services/pdf/endpoint_providers.py) implements `ModalEndpointProvider` (Modal owns autoscaling: auto-wake on first request, auto-drain after `scaledown_window`; warmup = `/health` probe, scale-to-zero = no-op) and `HuggingFaceEndpointProvider` (SDK resume / poll-for-GPU / scale-to-zero). Both serve the **identical** OpenAI-compatible vLLM `/v1/chat/completions` contract, so moving Surya between GPU hosts is one env var, no code change. `get_surya_config()` is provider-aware (resolves the effective URL + model id + token); the registry builds via `SuryaEndpointManager.from_config()`; `endpoint_controller` skips HF-SDK prep/drain for non-HF providers; `/health` + `surya_ocr_metrics.provider` report which host is active. **Modal became the default 2026-06-13** because HF's serverless pool repeatedly failed to allocate a GPU for the 650M model; HF is kept as a fallback (`SURYA_PROVIDER=huggingface`). The Surya-2 Modal app is deployed at `https://basilakis--surya-vllm-serve.modal.run` (app `surya-vllm`, workspace `basilakis`) — redeploy via [modal_app/surya_vllm.py](mivaa-pdf-extractor/modal_app/surya_vllm.py) (`modal deploy`; see [modal_app/README.md](mivaa-pdf-extractor/modal_app/README.md), which uses Modal's CUDA-base + pip-vLLM recipe). The only required runtime secret for the Modal path is `SURYA_MODAL_API_KEY` (the URL is baked as the config default); the HF fallback path uses `SURYA_ENDPOINT_URL`/`SURYA_ENDPOINT_NAME`/`SURYA_NAMESPACE` + `HUGGINGFACE_API_KEY`. All wired in `deploy.yml`.
- **Known residual**: Surya marks `Image`/`Figure` blocks `skipped:true` for OCR, so a product name rendered *inside* a photo is still not read (same blind spot as the old text-only discovery). **Follow-up (not yet built): Voyage `voyage-multimodal-3` page embedding → `vecs.page_embeddings` (8th fusion vector)** for catalog-Q&A retrieval.
...
- **Configuration**: uses the platform-wide `platform_secrets` registry (env-first, DB-fallback). Keys: `AADE_USERNAME`, `AADE_PASSWORD` (sensitive), `AADE_AFM_CALLED_BY` (optional non-sensitive). Admins manage at `/admin/modules/myaade → Settings`. Env vars on the function always win over the DB row; the source badge in the UI surfaces this.
