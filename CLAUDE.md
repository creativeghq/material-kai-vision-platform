...
- **`cron.job_run_details`** — `prune_cron_job_run_details()` (cron `cron-run-details-cleanup-daily`, 02:20): **succeeded 7d, failed/other 30d** (failures are diagnostic). **pg_cron never prunes its own run log** — this table had 5 months / 336k rows / 248 MB when first checked. With ~30 jobs, several running every minute, it grows unbounded forever. If you add a per-minute cron, this is what absorbs it.
- **`net._http_response`** (pg_net) — needs **no** retention rule; pg_net TTL-deletes its own rows (~1.5k live). But it **bloats**: it was 298 MB of dead space for 982 live rows because the file is never truncated. Not a leak, and not fixed by adding a cron — it needs an occasional `VACUUM FULL`. Same for `storage.objects`, whose *indexes* bloat from orphan-cleanup churn (18 MB → 144 kB via `REINDEX ... CONCURRENTLY`, which unlike `VACUUM FULL` takes no exclusive lock — always prefer it on user-facing hot tables).

**Distinguish the two failure modes before reaching for a fix:** unbounded *rows* need a retention cron; unbounded *disk against few rows* is bloat and needs `VACUUM FULL` / `REINDEX CONCURRENTLY`. A retention cron does nothing for bloat, and vacuuming does nothing for a table with no TTL.
...
- **TAXISnet audit notification**: every successful lookup writes an audit entry into the looked-up ΑΦΜ's TAXISnet inbox (per ΑΑΔΕ policy, can't be disabled). So ΑΑΔΕ is never the *first* place a name is looked for — bulk resolution goes to ΓΕΜΗ OpenData (free, silent, public). ΑΑΔΕ is called only when a **human acts on one specific business**: verifying their OWN business, or the operator-initiated "Add issuer to CRM" / company-research chain on a counterparty they are transacting with. The one automated caller is the inbound sync's issuer-name resolver ([resolve-issuer-names.ts](supabase/functions/_shared/finance/resolve-issuer-names.ts)) — **off by default**, per-workspace opt-in via `finance_settings.resolve_issuer_names_via_aade`, and bounded: only ΑΦΜ that ΓΕΜΗ has *definitively* 404'd (sole traders, who are not in ΓΕΜΗ and can be named nowhere else), only issuers who filed a document against that workspace, ≤5 per run, and an ΑΑΔΕ miss is cached as `source='aade'` so the same business is never notified twice. If you add another automated ΑΑΔΕ caller, it needs the same four bounds — the failure mode is silently notifying hundreds of businesses who never asked to hear from us.
- **RgWsPublic2 has ONE derivation**: envelope + `basic_rec` parse live in [_shared/aade/rgwspublic2.ts](supabase/functions/_shared/aade/rgwspublic2.ts), shared by the `myaade-rgwspublic2` route and the inbound resolver. Don't re-hand-roll the SOAP body in a new caller.
