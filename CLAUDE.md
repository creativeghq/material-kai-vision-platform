...
- **GPU host is a switch — HuggingFace OR Modal (`SURYA_PROVIDER`)**: [endpoint_providers.py](mivaa-pdf-extractor/app/services/pdf/endpoint_providers.py) implements `HuggingFaceEndpointProvider` (SDK resume / poll-for-GPU / scale-to-zero) and `ModalEndpointProvider` (Modal owns autoscaling: auto-wake on first request, auto-drain after `scaledown_window`; warmup = `/health` probe, scale-to-zero = no-op). Both serve the **identical** OpenAI-compatible vLLM `/v1/chat/completions` contract, so moving Surya between GPU hosts is one env var, no code change. `get_surya_config()` is provider-aware (resolves the effective URL + model id + token); the registry builds via `SuryaEndpointManager.from_config()`; `endpoint_controller` skips HF-SDK prep/drain for non-HF providers; `/health` + `surya_ocr_metrics.provider` report which host is active. **Modal was added 2026-06-13** because HF's serverless pool repeatedly failed to allocate a GPU for the 650M model. Deploy Surya to Modal via [modal_app/surya_vllm.py](mivaa-pdf-extractor/modal_app/surya_vllm.py) (`modal deploy`; see [modal_app/README.md](mivaa-pdf-extractor/modal_app/README.md)). Secrets — HF path: `SURYA_ENDPOINT_URL`/`SURYA_ENDPOINT_NAME`/`SURYA_NAMESPACE` (+ `HUGGINGFACE_API_KEY`); Modal path: `SURYA_PROVIDER=modal` + `SURYA_MODAL_URL` + `SURYA_MODAL_API_KEY`. All wired in `deploy.yml`.
