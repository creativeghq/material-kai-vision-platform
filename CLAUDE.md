...
11. **Output encoding.** No `dangerouslySetInnerHTML` / `document.write` / raw HTML-string assembly with user or AI content — use JSX/`react-markdown` (no `rehype-raw`) or the **canonical** escaper: `escapeHtml` from `supabase/functions/_shared/html.ts` (edge) or `src/utils/escapeHtml.ts` (frontend). Both escape the full `& < > " '` set (attribute-safe). Do NOT hand-roll a local copy — the old per-file copies drifted to 3 different strengths. `escapeHtml` is HTML-only: it is NOT a PostgREST filter sanitizer or a CSV quoter (separate contracts — never name them `esc`).
