...
Deno.serve(withApiLogging('hr-checkin-cron', async (req) => {
  ...
  const { data: settingsRows, error: settingsErr } = await supabase.from('hr_settings').select('*').eq('late_alert_enabled', true);
  // A failed read here is "no workspace has late alerts on", which is the silent-zero shape: the
  // run reports ok and nobody is ever told a soul was late (#354 HR-11).
  if (settingsErr) throw new Error(`hr_settings unreadable: ${settingsErr.message}`);
  let checked = 0, alerted = 0, unknown = 0;
  const now = new Date();
  ...
  for (const s of (settingsRows ?? [])) {
    ...
    const { data: emps, error: empErr } = await supabase.from('hr_employees')
    ...
    if (empErr) {
      // This workspace is skipped, and the count says so. Treating an unreadable roster as an
      // empty one would report a clean run for a workspace nobody checked.
      console.error('[hr-checkin] employees unreadable', { workspace: s.workspace_id, error: empErr.message });
      unknown++;
      continue;
    }
    ...
    for (const e of (emps ?? [])) {
      ...
      /**
       * Establish the FACT before charging for it (#354 HR-11).
       *
       * The punch lookup used to run after the debit, so an employee who clocked in at 09:03 still
       * cost the workspace a charge — and invariant 10 requires the debit to follow a confirmed
       * need. The read is also checked now: an errored lookup used to read as "no arrival", which
       * alerts an owner that someone is late when the truth is that we could not tell.
       */
      const { data: arr, error: arrErr } = await supabase.from('hr_time_punches').select('id')
        .eq('workspace_id', s.workspace_id).eq('employee_id', e.id).eq('reference_date', today)
        .eq('punch_type', 'arrival').limit(1).maybeSingle();
      if (arrErr) {
        console.error('[hr-checkin] punch lookup failed', { workspace: s.workspace_id, employee: e.id, error: arrErr.message });
        unknown++;
        continue;
      }
      if (arr) continue;

      // Genuinely late, and only now. One charge per workspace per run-with-alerts; no credits →
      // stop alerting for this workspace this run (nothing claimed yet; resumes when funded).
      if (!wsCharged) {
      ...
      if (claimErr) {
        // 23505 IS the guard working — this employee was alerted already. Anything else is a
        // failure to alert, and it must not be indistinguishable from one.
        if ((claimErr as { code?: string }).code !== '23505') {
          console.error('[hr-checkin] alert claim failed', { workspace: s.workspace_id, employee: e.id, error: claimErr.message });
          unknown++;
        }
        continue;
      }
  ...
  // `unknown` is the number of employees or workspaces this run could NOT decide about. A run that
  // alerted nobody because every read failed must not look like a run where nobody was late.
  return json({ ok: true, checked, alerted, unknown });
}));
