...
- **Two surfaces, one inbox.** *Documents → Expenses* is the DOCUMENT list (what arrived, filtered by source / detail / date). *Documents → By Supplier* (`?tab=expense_suppliers`) is the same inbox grouped by ISSUER, and it is a sibling tab rather than a panel on top of the list — as a panel it was the first thing on the Expenses screen and pushed the documents that tab exists to show below the fold. It reads `inbound_issuers_summary(workspace)`, the single derivation of per-issuer counts (`docs` / `unfiled` / `in_books`), money, span, the learned category **and** the CRM company the ΑΦΜ resolves to (matched on `crm_vat_norm`, #353 CRM-4 — 37 of 241 issuers today; the other 204 read **Not in CRM — add**, which opens `AddIssuerToCrmDialog`, the ONE add-issuer flow the document row menu also uses: duplicate probe on the normalised ΑΦΜ, then the ΑΑΔΕ → ΓΕΜΗ → web research chain. Never a silent create — the ΑΑΔΕ leg writes an audit entry into the issuer's own TAXISnet inbox, so it is opt-in per add. Filing is per supplier because that is the unit of the decision: 1,866 documents one at a time is 1,866 choices, by issuer it is 241, and the largest 45 carry 71% of the pile. `inbound_file_issuer` moves only documents still in a SYSTEM bucket and calls `remember_inbound_issuer_category`, so the next arrival files itself. Opening a row shows that supplier's whole run in a MODAL through `SupplierInboundDocs` — the same table, menu and dialogs their CRM record shows — and a document row inside it opens the AADE preview as a THIRD layer. Stacked, never swapped: both lists are long (241 suppliers, up to 206 documents each), so an inline expansion pushed the queue off screen, and every layer stays mounted so booking / receiving / paying a document returns to the same page of the same list. Closing the supplier modal refreshes the counts QUIETLY — work done inside it changes them, and a spinner there reads as the page reloading under the operator. - **Downstream**: `inbound_doc_to_supplier_bill(p_doc_id)` creates a `supplier_bills` row; `inbound_doc_receive_to_warehouse(p_doc_id, p_mappings)` records stock-in (see [warehouse doc](warehouse-and-billing.md)).
