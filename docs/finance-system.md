...
- **Inbound documents** — pulls **both** myDATA retrieval endpoints into `inbound_documents`: `RequestDocs` (what suppliers transmitted citing this workspace's VAT) and `RequestTransmittedDocs` (what **we** transmitted — every foreign purchase, plus rent and payroll). Convert to `supplier_bills` and/or receive into warehouse stock.
...
Pulls **both** of myDATA's REST retrieval feeds. They answer different questions and only one of them was ever wired (issue #377):

| endpoint | returns | lands as |
|---|---|---|
| `RequestDocs` | documents **other** businesses submitted citing this workspace's VAT as counterpart | `source='mydata'` |
| `RequestTransmittedDocs` | documents **we** submitted | `source='mydata_self'` |

A foreign supplier is not a myDATA obligor, so nobody files their invoice against us — the finance team types it into myAADE as a `14.x`, which makes **us** the transmitter. Those documents therefore land on the second endpoint and are permanently invisible to the first. Polling only `RequestDocs` hid EUR 45,413.93 of foreign purchases across 12 suppliers and 5 countries for two years, plus EUR 104,750.07 of self-reported rent (`16.1`) and payroll (`17.x`). Nothing failed; the question was wrong.
...
- **Pull**: `GET {baseUrl}/{RequestDocs|RequestTransmittedDocs}?mark={watermark}` with `aade-user-id` + `Ocp-Apim-Subscription-Key` headers. **Separate watermarks** — `finance_settings.inbound_last_mark` and `.inbound_last_transmitted_mark` — because MARKs come from one global AADE sequence and a shared watermark would let whichever branch ran first drag the other past documents it never saw.
- **Pagination is mandatory.** Both endpoints truncate at ~100 documents and return a `<continuationToken>` (`nextPartitionKey` + `nextRowKey`) that must be echoed back. Page one carries no marker distinguishing it from a complete result: the 2025 window returns 103 documents and a token, and page two holds 50 more. `fetchAllDocPages` follows every page and **reports** hitting its 40-page cap rather than truncating silently.
- **Transmitted branch takes expenses only** — families `13`/`14` (foreign, reverse-charged), `16` (rent), `17` (payroll). Everything else that endpoint returns is a sale we issued and already lives in `invoices`; those advance the watermark and are counted in the run summary under `skipped_own_sales`, never silently dropped.
- **Reverse charge is never a payable.** On `13.x`/`14.x` the acquisition is zero-rated at source and the VAT is self-accounted, so `_inbound_doc_to_supplier_bill_core` books `subtotal_net = total = net, vat_amount = 0`. The myDATA VAT figure stays on `inbound_documents.total_vat` for the VAT return. Guarded by `dic_detect__finance_reverse_charge_booked_at_gross`.
- **Payroll is never a supplier bill** — `17.x` is visible here but belongs to the HR module; the RPC refuses it and the row's action menu does not offer it.
...
- **Lands in** `inbound_documents` (upsert ignore-duplicates on `(workspace_id, source, mark)`). `status`: `new` → `classified` → `received` (lines mapped to stock) / `dismissed`. `lines` jsonb holds per-line `{line_number, quantity, net_value, vat_amount, item_description}`.
- **`mark`/`uid`/`authentication_code`/`qr_code_url` are GENERATED columns** reading out of `source_ref jsonb`, which is the single writable home. A MARK is myDATA's way of naming a document, not the document's identity — post-2030 ViDA documents will not have one. `reported_to_mydata` is likewise derived from `mydata_mark`, never stored beside it.
- **Two provenance facts, kept apart.** `source` (where the money record came from — permanent) and `lines_source` (`mydata` | `user` | `document` | `none` — whether the lines name anything, and it changes). A `14.x` carries exactly one value-only line, and so does most `2.x` Greek service billing: 1,161 of 1,769 rows are `lines_source='none'`. That flag is what gates warehouse receive and AI product extraction, and it is why they correctly skip these documents rather than inventing stock.
...
- **Two surfaces, one inbox.** *Documents → Expenses* is the DOCUMENT list (what arrived, filtered by source / detail / date). *Documents → By Supplier* (`?tab=expense_suppliers`) is the same inbox grouped by ISSUER, and it is a sibling tab rather than a panel on top of the list — as a panel it was the first thing on the Expenses screen and pushed the documents that tab exists to show below the fold. It reads `inbound_issuers_summary(workspace)`, the single derivation of per-issuer counts (`docs` / `unfiled` / `in_books`), money, span, the learned category **and** the CRM company the ΑΦΜ resolves to (matched on `crm_vat_norm`, #353 CRM-4 — 37 of 241 issuers today; the other 204 read **Not in CRM — add**, which opens `AddIssuerToCrmDialog`, the ONE add-issuer flow the document row menu also uses: duplicate probe on the normalised ΑΦΜ, then the ΑΑΔΕ → ΓΕΜΗ → web research chain. Never a silent create — the ΑΑΔΕ leg writes an audit entry into the issuer's own TAXISnet inbox, so it is opt-in per add. Filing is per supplier because that is the unit of the decision: 1,866 documents one at a time is 1,866 choices, by issuer it is 241, and the largest 45 carry 71% of the pile. `inbound_file_issuer` moves only documents still in a SYSTEM bucket and calls `remember_inbound_issuer_category`, so the next arrival files itself. Opening a row shows that supplier's whole run in a MODAL through `SupplierInboundDocs` — the same table, menu and dialogs their CRM record shows — and a document row inside it opens the AADE preview as a THIRD layer. - **Every action a received document has is wired ONCE, in `useInboundDocActions`.** The hook owns the state (`busy`, the receive / complete-lines / order / payment dialogs, the `ordered` set), constructs the menu, and returns `renderActions(doc)` + `dialogs`; both tables — the Expenses inbox and `SupplierInboundDocs` (CRM company card + the by-supplier modal) — render those and keep only their own COLUMNS and row source. The reason it is one place: several menu entries are gated on the handler existing (`!!onAddLineDetail`), so a table that spells the props out itself and forgets one DELETES that entry rather than disabling it. Both tables used to spell them out, they drifted, and **Add line detail** was unreachable from two of the three surfaces — on two thirds of received documents (value-only lines, 1,161 of 1,769 here) that was the only way to complete them. Pinned by [tests/unit/inboundDocActionsParity.test.ts](../tests/unit/inboundDocActionsParity.test.ts): the menu is constructed in exactly ONE file, that file passes every gating handler, and both tables actually use it — each half is defeatable alone. The list itself filters through the shared engine (`useFilters` + `FilterBar`, same as Parties): search, to-file, in-CRM, in-books, filing rule, first/last activity window, document count, net total. Inside the modal an **issue-date window** narrows the supplier's own run, applied SERVER-side so a narrow window reaches documents the 500-row cap would have cut off; it is EMPTY on open and bounded by the span the summary derived, because a prefilled window is a filter nobody set — it would drop any document carrying no issue date and put the header's all-time counts silently at odds with the table. A host that hides itself on an empty list must read `counts.windowed` (`PartyInboundDocsCard` does): narrowing to a quiet month otherwise deletes the panel, and with it the only control that could widen the window again. When the supplier is in CRM the modal also links their **account card** (`?tab=parties&party=company:<id>`) — bills, orders, payments, running ledger and the emailed statement; offered only once the ΑΦΜ resolves, since that page is keyed on the party. Above the table sit **Invoiced / In the books / Paid** for the window in force, derived by `inbound_issuer_money` — never summed from the loaded page, which is 20 rows wearing the label of a supplier's whole history. `paid` only exists for documents that became a bill, so `booked_documents` is returned beside it and rendered under it: 206 documents, EUR 37,525.96 invoiced and nothing paid is a to-do, not a debt, and a bare "EUR 0 paid" says the wrong one. A failed read renders as UNKNOWN, a mixed-currency window refuses to total, and the strip shows "Totalling…" on every refetch rather than pairing the previous window's money with the new window's label.
- **`inbound_invoiced_total(doc_type, net, gross)` is the one SQL statement of "what did this cost".** Reverse-charged 13.x/14.x carry no supplier VAT — the myDATA gross adds OUR self-assessed entry, money that never moves — so those count at NET. The rule was written twice (inline in `_inbound_doc_to_supplier_bill_core`, and as `invoicedTotal()` in TS); the booking writer now calls the function, verified equivalent across all 1,866 live documents before the swap. The TS twin stays because it formats a row the browser already holds, and is pinned to the same rule. Stacked, never swapped: both lists are long (241 suppliers, up to 206 documents each), so an inline expansion pushed the queue off screen, and every layer stays mounted so booking / receiving / paying a document returns to the same page of the same list. Closing the supplier modal refreshes the counts QUIETLY — work done inside it changes them, and a spinner there reads as the page reloading under the operator. - **Downstream**: `inbound_doc_to_supplier_bill(p_doc_id)` creates a `supplier_bills` row; `inbound_doc_receive_to_warehouse(p_doc_id, p_mappings)` records stock-in (see [warehouse doc](warehouse-and-billing.md)).
