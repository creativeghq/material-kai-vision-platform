...
**Phase 2 (shipped 2026-07-24) — regenerable deliverable PDFs.** The sweep also purges **quote**, **presentation-sheet** and **client-view** PDFs. Trigger: PDF idle > **7 days** (age measured from `pdf_generated_at` so a rebuilt PDF isn't immediately re-purged), OR sheets when their `moodboards.status = 'completed'`, OR client-views when their `projects.status IN ('completed','done','closed','archived')`. Purge nulls `pdf_storage_path` (+ stamps `files_purged_at`) so the file drops from the reference set and the nightly GC reclaims it. **Every read path rebuilds on demand, credit-free** (re-render only debits on `action:'create'`): the service layer (`QuotePDFService.refreshPDFUrl`, `moodboardSheetsService.refreshPdfUrl`, `clientViewsService.refreshPdfUrl`) regenerates when it sees a purged row; the public share endpoints do it server-side via the service-role key — `moodboard-sheet-share` (`ensureDeliverablePdf` for sheet + client-view) and `quote-public-share` (on the `download` event; the quote page otherwise renders from data). A moodboard **"Mark as done"** control (`moodboardAPI.setMoodBoardStatus`) sets the status that frees its sheet PDFs. **Presentation-catalog PDFs are covered too** (idle > 7 days): `catalogsService.refreshPdfUrl` rebuilds for the owner and the public gate (`catalog-access` `verify`) rebuilds server-side via the service-role key — the catalog page renders from `body_data` regardless. So no regenerable-PDF class is kept forever. Finance/tax PDFs (invoices, credit/delivery notes, statements) and KB source PDFs are **hard-exempt**.
