...
**Environment variables**: `SLIG_ENDPOINT_URL`, `SLIG_ENDPOINT_TOKEN` (the SLIG visual-embedding endpoint on HuggingFace). The structural pass (PaddleOCR-VL) runs on Modal — its only required runtime var is `PADDLEOCR_MODAL_API_KEY` (the URL is baked as a config default; see the PaddleOCR-VL section below).
