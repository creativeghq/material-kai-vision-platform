...
- **Retry strategy**: 3 attempts at temperatures 0.0 / 0.4 / 0.8 (jittering breaks the model's sticky-prose state, lifts success rate to >95%; widened from 0.0/0.1/0.2 in 2026-05-03 — the narrow spread kept the model stuck in sticky-prose state through all three retries on graphic-heavy pages)
...
### OCR
1. **Chandra v2 with retry-jitter** (3 attempts, temps 0.0/0.4/0.8)
...
- ✅ **Chandra v2** as sole OCR engine, with retry-with-jitter (3 attempts at temps 0.0/0.4/0.8)
