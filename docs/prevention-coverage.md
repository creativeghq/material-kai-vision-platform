...
- **Then done anyway (#358, 2026-08-17):** the column-level revoke was first ruled out because admins read cost through the same `authenticated` role — and then applied, with the sell side reading cost through `get_product_costs()` / `get_order_item_costs()` / `get_invoice_item_costs()` (DEFINER, gated on `is_workspace_sell_side`) instead of the table. `products`, `order_items` and `invoice_items` now carry a column allowlist and no table-level SELECT.
- **The trap that created:** a column ADDED to one of those tables is readable by nobody, and PostgREST refuses the whole request naming it (`42501`). `products.profile_user_id` (2026-09-05) broke Profile → Services within hours; `order_items.configured_options` (2026-08-29) sat in `ORDER_ITEM_SELECT` for a week while `ordersService.get` destructured `{ data }` without the error, so every order detail rendered with **no lines** — the silent-zero shape, from a *permission* error. Guarded by `lint_column_grants()` (the `db.column-grants` smoke check, strict zero): every ungranted column on an allowlisted table is a finding unless its `COMMENT ON COLUMN` says "NOT selectable". Proven to fire 2026-09-05 in a rolled-back probe: `REVOKE SELECT (profile_user_id) ON products FROM authenticated` made it return exactly `products.profile_user_id`; the live roster is zero. Blind spot: it watches `authenticated` only — `anon`'s narrower lists (`kb_docs`) are public projections by design and a column missing from one is the safe default, not a defect.
