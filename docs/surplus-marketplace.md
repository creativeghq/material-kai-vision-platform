...
Both parties then converse in the seller's shared Inbox (see [api/messaging-api.md](./api/messaging-api.md) and the `inbox-api` entry in [api/openapi-edge.json](./api/openapi-edge.json)). The frontend navigates the buyer to `/inbox?thread={thread_id}`.
