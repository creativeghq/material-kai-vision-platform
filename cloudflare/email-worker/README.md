...
3. **Plan.** Workers **Paid** ($5/mo). The Free plan can fail a handler that streams a 20 MiB body.
4. **Secrets.**
   ```bash
...
```
basilis@mail.materialshub.gr        first Basilis
basilis2@mail.materialshub.gr       the next one
```

An earlier draft mandated a random suffix for enumeration resistance. That does not survive contact
with the product — this is an address you print on a business card. What protects it is
`setReject()` on unknown recipients, the DKIM gate and per-sender rate limits, not obscurity.
